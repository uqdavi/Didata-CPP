...
void executarLinha(vector<string> tokens, map<string, double>& variaveis, bool mostrarLogs) {
    ...
    if (atribuicao(tokens, variaveis, mostrarLogs)) {
        return;
    }                
    // Variavel, ex: X <- 10

    
    if (escreva(tokens, variaveis)) {
        return;
    }
 // Variavel com operacao, ex: X <- 10 + 20

   if (atribuicaoOperacao(tokens, variaveis, mostrarLogs)) {
        return;
    } // Escreva, ex: escreva(X)

    if (escrevaOperacao(tokens, variaveis)) {
        return;
    } // Escreva com Operacao, ex: escreva(X+10)
